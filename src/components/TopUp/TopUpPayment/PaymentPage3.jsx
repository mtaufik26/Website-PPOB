...
const PaymentPage3 = () => {
  ...
  const handleButtonClick = (method) => {
    if (method.id === 'linkaja') {
      setError('Tidak bisa menggunakan metode ini');
    } else {
  ...
  {paymentMethods.map((method) => (
    <div key={method.id}>
      <button
        className={`w-full p-4 flex items-center justify-between text-left rounded-lg transition-all duration-200 transform hover:scale-105 ${
          method.color
        } hover:shadow-lg focus:outline-none focus:ring-2 ${
          method.id === 'linkaja' ? 'cursor-not-allowed opacity-50' : 'focus:ring-sky-500'
        } ${
          selectedMethod === method.id ? 'ring-2 ring-sky-600' : ''
        }`}
        onClick={() => handleButtonClick(method)}
        aria-label={`Pilih ${method.name}`}
        disabled={method.id === 'linkaja'}
      >
        <div className="flex items-center space-x-4">
          <span className="text-3xl">{method.icon}</span>
          <span className={`font-medium ${method.id === 'linkaja' ? 'text-gray-400' : 'text-gray-700'}`}>
            {method.name}
          </span>
        </div>
        <svg
      ...
      {method.id === 'linkaja' && (
        <div className="text-sm text-red-500 mt-2">
          Tidak tersedia pada pembayaran ini
        </div>
      )}
    </div>
  ))}
